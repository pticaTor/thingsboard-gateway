...
class JsonPathMqttUplinkConverter(MqttUplinkConverter):
    ...
    def convert(self, config, data):
        ...
        try:
            if self.__config.get("deviceNameJsonExpression") is not None:
                dict_result["deviceName"] = TBUtility.get_path_value(self.__config.get("deviceNameJsonExpression"), data)
            ...
            if self.__config.get("deviceTypeJsonExpression") is not None:
                dict_result["deviceType"] = TBUtility.get_path_value(self.__config.get("deviceTypeJsonExpression"), data)
        ...
        try:
            if self.__config.get("attributes"):
                for attribute in self.__config.get("attributes"):
                    dict_result["attributes"].append({attribute["key"]: TBUtility.get_path_value(attribute["value"],
                                                                                                 data,
                                                                                                 attribute["type"])})
        except Exception as e:
            log.error('Error in converter, for config: \n%s\n and message: \n%s\n', dumps(self.__config), data)
        ...
        try:
            if self.__config.get("timeseries"):
                for ts in self.__config.get("timeseries"):
                    dict_result["telemetry"].append({ts["key"]: TBUtility.get_path_value(ts["value"],
                                                                                         data,
                                                                                    ts["type"])})
        except Exception as e:
            log.error('Error in converter, for config: \n%s\n and message: \n%s\n', dumps(self.__config), data)
